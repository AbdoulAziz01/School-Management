...
<div class="container-fluid">
    <p class="mb-3">
        <a href="{{ route('admin.teachers.import.mapping', $token) }}" class="text-decoration-none small">
    ...
    <div class="card">
        ...
        <div class="card-footer bg-white d-flex gap-2">
            <form method="POST" action="{{ route('admin.teachers.import.store', $token) }}"
                  onsubmit="return confirm('Importer {{ $validCount }} enseignant(s) ? Cette action créera directement les comptes.');">
                @csrf
                <button type="submit" class="btn btn-primary" {{ $validCount === 0 ? 'disabled' : '' }}>
                    <i class="fas fa-check me-1"></i> Importer {{ $validCount }} enseignant(s) valide(s)
                </button>
            </form>
            <a href="{{ route('admin.teachers.import') }}" class="btn btn-outline-secondary">Annuler</a>
        </div>
    </div>
</div>
@endsection
