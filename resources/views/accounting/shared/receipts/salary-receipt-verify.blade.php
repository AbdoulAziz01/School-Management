    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            <table class="table table-bordered table-sm mb-0">
                <tr><th style="width:40%">Employé</th><td>{{ $salaryPayment->user->name }}</td></tr>
                <tr><th>Période</th><td>{{ $salaryPayment->period->locale('fr')->translatedFormat('F Y') }}</td></tr>
                <tr><th>Date de paiement</th><td>{{ $salaryPayment->paid_at?->format('d/m/Y H:i') ?? '—' }}</td></tr>
                <tr><th>Montant dû</th><td>{{ number_format($salaryPayment->amount_due, 0, ',', ' ') }} FCFA</td></tr>
                <tr><th>{{ $salaryPayment->status === 'partial' ? 'Total payé à ce jour' : 'Montant payé' }}</th><td>{{ number_format($salaryPayment->amount_paid, 0, ',', ' ') }} FCFA</td></tr>
                @if($salaryPayment->status === 'partial')
                    <tr><th>Reste à payer</th><td>{{ number_format($salaryPayment->balanceDue(), 0, ',', ' ') }} FCFA</td></tr>
                @endif
                <tr><th>Payé par</th><td>{{ $salaryPayment->paidBy->name ?? '—' }}</td></tr>
            </table>
        </div>
    </div>
</div>
